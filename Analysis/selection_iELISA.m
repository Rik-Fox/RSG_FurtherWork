...
for itr=61:length(Algor_varient)-1
    
    if ismember(Algor_varient(itr),CNS)
        hzname = 'YasaBonga';
%         for hz =1:healthzone
%             if hz == 1
%                 hzname = 'YasaBonga';
...
%             
            ...
            subplot(2,1,2)
            hold on
            stairs(Year,ActiveCases1+ActiveCases2,'Color','r')
            stairs(Year,PassiveCases1+PassiveCases2,'Color','b')           
            stairs(Year,ActiveCases1+ActiveCases2+PassiveCases1+PassiveCases2,'Color','k')
            stairs(Year,NewInfections,'Color',[0 0.7 0]);
