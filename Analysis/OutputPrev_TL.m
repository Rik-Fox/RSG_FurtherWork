...
for itr=1:length(Algor_varient)
    for hz =1:healthzone
        ...
        for scr=3:screentype
            ...
            if mini == true
                load("Class_dataODEmini/ClassesTL_"+string(Algor_type(itr))+"_"+hzname+'_'+scrname+'_'+string(Algor_varient(itr))+".mat",'Classes');
                load("Agg_dataODEmini/AggregateTL_"+string(Algor_type(itr))+"_"+hzname+'_'+scrname+'_'+string(Algor_varient(itr))+".mat",'Aggregate');
                load("intervent_dataODEmini/interventionTL_"+string(Algor_type(itr))+"_"+hzname+'_'+scrname+'_'+string(Algor_varient(itr))+".mat",'intervention');
            else
...
csvwrite('prevTL.csv', prevs);
csvwrite('NumScreenTL.csv',NumScreens);